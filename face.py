...
# Detect key points with dlib
def get_image_points(img):
    dets = detector(img, 0)###0 represents no enlarge opporation; if 1, image size is double/
...
def get_pose_estimation(image, image_points):
    # 3D model points.
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        _, frame = cap.read()
        image_points = get_image_points(frame)
        if image_points is None:
          #  print('get_image_points failed')
            continue
        ret, rotation_vector, translation_vector, camera_matrix, dist_coeffs = get_pose_estimation(frame, image_points)
        if not ret:
